...
namespace Justin_Marshall___Benchmark_Assignment
{
    public static class SearchAlgos
    {
        ...
        {
            //sort movie list by ID and then binary search
            //return matching movie or null if not found
            List<Movie> currentMovies = movieList.ToList();
            //sorts data for binary search using built-in compare method
            currentMovies.Sort((a, b) => string.Compare(a.ID, b.ID));

            int left = 0;
            //right is last valid index postion
            ...
            { 
                ...
                //if ID matches return movie object
                if (compare == 0)
                {
                    return currentMovies[mid];
                }//if middle ID is smaller than target search right half
                else if (compare < 0)
                {
                    left = mid + 1;
                }//if middle ID is greater than target search left half
                else
                { right = mid - 1; }
            }
            return null;
